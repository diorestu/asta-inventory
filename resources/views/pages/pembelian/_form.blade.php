...
<form action="{{ isset($data) ? route('pembelian.update', $data->id) : route('pembelian.store') }}" method="POST"
    id="productForm">
    @csrf
    ...
    <div>
        <div class="float-end w-25">
            <x-input type="date" name="request_date" label="Tanggal Permintaan" :data="$data->request_date ?? date('Y-m-d')" />
        </div>
